                ...
                {groups.map((g, i) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className={`flex flex-col md:flex-row items-center justify-between p-6 md:p-8 bg-black/40 border-2 transition-all relative overflow-hidden group ${cardStyles}`}
                        >
                            <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-8 w-full md:w-auto text-center sm:text-left">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <div className="flex flex-wrap justify-center sm:justify-start gap-2 mt-2 sm:mt-1">
                                        <span className={`${isPublic ? 'bg-white text-black' : 'bg-red-600 text-white'} text-[8px] font-black px-2 py-1`}>{g.status}</span>
                                        {subscriptions[g.id] && <span className="bg-green-600 text-[8px] font-black px-2 py-1">SUBSCRIBED</span>}
                                    </div>
                                </div>
                            </div>
                            <button onClick={() => g.link && window.open(g.link, '_blank')} className="w-full md:w-auto mt-6 md:mt-0 px-8 md:px-12 py-4 bg-white text-black font-black text-[10px] tracking-widest uppercase hover:bg-red-600 hover:text-white transition-all transform skew-x-0 md:skew-x-[-10deg]">JOIN NOW</button>
                        </motion.div>
                    );
                })}
            </div>
