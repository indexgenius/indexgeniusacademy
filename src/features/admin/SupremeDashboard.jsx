            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 border-b border-white/5 pb-8">
                <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] font-black text-red-600 tracking-[0.4em] uppercase">SUPREME COMMANDER PROTOCOL v7.0</span>
                    </div>
                    <h2 className="text-4xl lg:text-7xl font-black italic tracking-tighter text-white uppercase leading-none">
                        SYSTEM <span className="text-red-600">OVERRIDE</span>
                    </h2>
                    <p className="text-[9px] font-black text-gray-500 tracking-[0.2em] uppercase mt-4 flex items-center gap-3">
                        <Activity size={12} className="text-green-500" /> KERNEL: <span className="text-white">ONLINE</span>
                        <Database size={12} className="text-blue-500 ml-4" /> REPLICA: <span className="text-white">SYNCED</span>
                        <Terminal size={12} className="text-red-600 ml-4" /> ADMIN: <span className="text-white">JEILIN_SUPREME</span>
                    </p>
                </div>

                <div className="flex flex-wrap gap-2 bg-white/5 p-1 border border-white/10 w-full lg:w-auto">
                    {[
                        ...
                        <button
                            ...
                            className={`flex-1 lg:flex-none px-4 lg:px-6 py-2.5 lg:py-3 text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${view === t.id ? 'bg-red-600 text-white' : 'text-gray-500 hover:text-white'}`}
                        >
                            <t.icon size={12} className="lg:w-3.5 lg:h-3.5" /> {t.id}
                        </button>
                    ))}
                </div>
            </div>
                                ...
                                <>
                                    {filteredUsers.slice(0, visibleCount).map(u => (
                                        ...
                                        >
                                            ...
                                            {/* SUPREME OVERRIDE BUTTONS */}
                                            <div className="grid grid-cols-2 gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity mt-2 lg:mt-0">
                                                {u.status !== 'approved' && (
                                                    <button
                                                        onClick={() => handleSupremeApprove(u)}
                                                        className="py-2 bg-green-600 text-white text-[8px] font-black uppercase tracking-widest hover:bg-white hover:text-green-600 transition-all flex items-center justify-center gap-2"
                                                    >
                                                        <ShieldCheck size={10} /> AUTHORIZE
                                                    </button>
                                                )}
                                                <button
                                                    onClick={() => handleSupremeDelete(u)}
                                                    className="py-2 bg-white/10 text-red-600 text-[8px] font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all flex items-center justify-center gap-2 border border-red-600/20"
                                                >
