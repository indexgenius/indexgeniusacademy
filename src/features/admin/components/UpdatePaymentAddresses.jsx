...
const UpdatePaymentAddresses = () => {
    ...
    const handleUpdate = async () => {
        setStatus('loading'); setMessage('ACTUALIZANDO DIRECCIONES...'); setDetails([]);
        const logs = [];
        try {
            const ref = collection(db, "payment_methods");
            const q = query(ref, where("category", "==", "CRIPTO"));
            const snap = await getDocs(q);
            for (const d of snap.docs) { await deleteDoc(doc(db, "payment_methods", d.id)); logs.push(`ELIMINADO: ${d.data().name}`); }
            for (const m of newPaymentMethods) { await addDoc(ref, m); logs.push(`AÑADIDO: ${m.name}`); }
            setStatus('success'); setMessage('COMPLETADO'); setDetails(logs);
        } catch (e) { setStatus('error'); setMessage(e.message); }
    };

    return (
        <div className="bg-white/5 border border-white/10 p-6">
            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><RefreshCw size={20} className="text-red-600" /> ACTUALIZAR DIRECCIONES CRIPTO</h3>
            <button onClick={handleUpdate} disabled={status === 'loading'} className={`w-full py-4 font-black uppercase tracking-widest text-white transition-all flex items-center justify-center gap-2 ${status === 'loading' ? 'bg-gray-600' : 'bg-red-600'}`}>
                {status === 'loading' ? 'PROCESANDO...' : 'INICIAR ACTUALIZACIÓN'}
            </button>
