...
const PaymentControl = () => {
    ...
    const ICON_OPTIONS = [
        { value: 'Wallet', label: 'DEFAULT WALLET' },
        { value: 'Landmark', label: 'BANK LOGO' },
        { value: 'Smartphone', label: 'MOBILE APP' },
        { value: 'usdt', label: 'USDT LOGO' },
    ...
    const handleDelete = async (id) => {
        if (!confirm('TERMINATE THIS PAYMENT CHANNEL?')) return;
        await deleteDoc(doc(db, "payment_methods", id));
    };

    const seedTacticalPayments = async () => {
        if (!confirm('FORCE SEED DEFAULT PAYMENT DATA?')) return;
        const defaults = [
        ...
        alert('SYSTEM: 6 PAYMENT METHODS INITIALIZED');
    };

    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-8">
            <div className="space-y-6">
                <h3 className="text-sm font-black text-gray-500 uppercase tracking-widest">{pmForm.id ? 'EDIT CHANNEL' : 'ADD PAYMENT CHANNEL'}</h3>
                <form onSubmit={handleSubmit} className="space-y-4 bg-black border border-white/5 p-6 shadow-red-glow/5 relative">
                    ...
                    />
                    <input placeholder="METHOD NAME" value={pmForm.name} onChange={e => setPmForm({ ...pmForm, name: e.target.value.toUpperCase() })} className="w-full bg-white/5 border border-white/10 p-4 text-xs font-bold text-white outline-none" />
                    <input placeholder="VALUE / ADDRESS" value={pmForm.value} onChange={e => setPmForm({ ...pmForm, value: e.target.value })} className="w-full bg-white/5 border border-white/10 p-4 text-xs font-bold text-white outline-none" />
                    <input placeholder="OWNER NAME" value={pmForm.owner} onChange={e => setPmForm({ ...pmForm, owner: e.target.value.toUpperCase() })} className="w-full bg-white/5 border border-white/10 p-4 text-xs font-bold text-white outline-none" />
                    <TacticalSelect
                    ...
                    <button disabled={pmLoading} className={`w-full py-4 text-white text-xs font-black tracking-widest uppercase hover:bg-white hover:text-black transition-all ${pmForm.id ? 'bg-blue-600' : 'bg-red-600'}`}>
                        {pmLoading ? 'SYNCHRONIZING...' : pmForm.id ? 'UPDATE CHANNEL' : 'AUTHORIZE CHANNEL'}
                    </button>
                    {pmForm.id && (
                        <button type="button" onClick={() => setPmForm({ category: 'CRIPTO', name: '', value: '', owner: '', icon: 'Wallet' })} className="w-full py-2 text-[8px] font-black text-gray-500 hover:text-white transition-colors">[ CANCEL EDIT ]</button>
                    )}
                    <button type="button" onClick={seedTacticalPayments} className="w-full text-[9px] font-black text-gray-700 hover:text-red-500 transition-colors pt-4 uppercase tracking-[0.3em]">[ INITIALIZE TACTICAL PAYMENTS ]</button>
                </form>
            </div>

            <div className="space-y-6">
                <h3 className="text-sm font-black text-gray-500 uppercase tracking-widest">SYSTEM AUDIT: ACTIVE METHODS</h3>
                        ...
                        <div key={pm.id} className="p-5 bg-white/5 border border-white/10 flex justify-between items-center group relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-4 pl-2">
                                <div className="w-10 h-10 bg-black border border-white/10 flex items-center justify-center p-1">
                                    {pm.icon === 'bancolombia' ? <img src="/img/metodos/logos/Logo_Bancolombia.svg.png" className="w-full h-full object-contain" alt="BAN" /> :
